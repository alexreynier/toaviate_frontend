...
jsTag.factory('InputService', ['$filter', 'KeywordsService', function($filter, KeywordsService) {
  ...
  InputService.prototype.breakCodeHit = function(tagsCollection, options) {
    if (this.input !== "") {
      ...
      if(originalValue === parseInt(originalValue)){
        //this means that we've already got an ID in the database
        console.log("INTEGER");
        KeywordsService.GetAll()
            .then(function (data){
              var pieces = data;
              var result = $.grep(data, function(e){ return e.id == originalValue; });
              var keyword = result[0].keyword;
              var keyword = keyword.toLowerCase();
              console.log(keyword);
              tagsCollection.addTag(keyword);
        });
        //we need to GET the content for this ID one...


      } else {
          //this means that we don't have an ID in the database WE NEED TO ADD IT first
