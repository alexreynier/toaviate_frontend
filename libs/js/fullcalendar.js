...
var EventRenderer = FC.EventRenderer = Class.extend({
	...
	getSkinCss: function(eventDef) {
		// console.log("EVENT DEF ", eventDef);
		// console.log("MODIF" ,this.getBgColor(eventDef));
		return {
			'background': this.getBgColor(eventDef),
			'border-color': this.getBorderColor(eventDef),
			'opacity': this.getOpacity(eventDef),
	...
	getOpacity: function(eventDef) {
		var val;

		if(eventDef && eventDef.miscProps && eventDef.miscProps.opacity){
			val = eventDef.miscProps.opacity;
		}
	...
	getStylingObjs: function(eventDef) {
		...
		// console.log("getstylingobjs = ", objs);
		return objs;
	},
...
var EventSource = Class.extend(ParsableModelMixin, {
	...
	*/
	...
	parseEventDefs: function(rawEventDefs) {
		...
		var eventDef;
		var eventDefs = [];
		if(rawEventDefs && rawEventDefs.length > 0){
...
JsonFeedEventSource.parse = function(rawInput, calendar) {
	...
	// normalize raw input
	if (rawInput && rawInput.url && typeof rawInput.url === 'string') { // extended form
		rawProps = rawInput;
	}
	else if (rawInput && typeof rawInput === 'string') { // short form
