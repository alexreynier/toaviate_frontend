 app.controller('DashboardClubPropellerLogbookController', DashboardClubPropellerLogbookController);
    ...
    function DashboardClubPropellerLogbookController(UserService, PlaneService, $rootScope, $location, $scope, $state, $stateParams, $uibModal, $log, $window, LicenceService, MedicalService, DifferencesService, PlaneDocumentService, $http, ToastService, LogbookLinkService, WorkpackService, LogbookHoursCorrectionService, LogbookExportService) {
        ...
        // ══════════════════════════════════════════════════════════
        // LOGBOOK HOURS CORRECTION (PROPELLER)
        // ══════════════════════════════════════════════════════════
        ...
        vm.lhc_history_filter = 'propeller';
        ...
        vm.lhcSubmit = function() {
            ...
            var payload = {
                ...
                propeller_id: parseInt($stateParams.prop_id),
            ...
            LogbookHoursCorrectionService.CorrectPropellerHours(payload)
                .then(function(data) {
                    vm.lhc_submitting = false;
                    if (data.success) {
                        vm.lhc_result = data.correction;
                        vm.lhc_step = 'result';
                        ToastService.success('Hours Corrected', 'Propeller hours have been corrected successfully. ' + data.correction.entries_recalculated + ' entries recalculated.');
                        PlaneService.GetPropLogs($stateParams.prop_id, $stateParams.plane_id, 0, 25)
                            .then(function(logData) {
                                vm.logs = logData.logs;
                                vm.propeller = logData.propeller;
                                vm.current_offset = 0;
                                vm.all_loaded = false;
                            });
                    } else {
